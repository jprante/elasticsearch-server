...
public class JarHell {
     ...
     */
    public static Set<URI> parseClassPath()  {
    ...
    static Set<URI> parseClassPath(String classPath) {
        String pathSeparator = System.getProperty("path.separator");
        String fileSeparator = System.getProperty("file.separator");
        String elements[] = classPath.split(pathSeparator);
        Set<URI> uriElements = new LinkedHashSet<>(); // order is already lost, but some filesystems have it
        for (String element : elements) {
            ...
            // Note: under Java Module Platform, the class path is empty. Continue and accept such elements.
            if (element.isEmpty()) {
                continue;
            ...
            URI uri = PathUtils.get(element).toUri();
            if (uriElements.add(uri) == false) {
     ...
     * @param uris A set of URLs from the classpath to be checked for conflicting jars
     * @param output A {@link String} {@link Consumer} to which debug output will be sent
     * @throws IllegalStateException if jar hell was found
     */
    @SuppressForbidden(reason = "needs JarFile for speed, just reading entries")
    public static void checkJarHell(Set<URI> uris, Consumer<String> output) throws IOException {
        ...
        for (final URI uri : uris) {
            final Path path = PathUtils.get(uri);
            ...
            if (path.toString().endsWith(".jar")) {
                ...
                final Path root = PathUtils.get(uri);
